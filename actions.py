...
class ItemAction(Action):
    def __init__(
        ...
        self.item = item
        if not target_xy:
            target_xy = int(entity.x), int(entity.y)
        #self.target_xy = target_xy
        ### HACKY FIX FOR FLOAT ISSUE
        intermediateX, intermediateY = target_xy
        self.target_xy = int(intermediateX), int(intermediateY)
...
class MeleeAction(ActionWithDirection):
    def perform(self) -> None:
        ...
        attack_desc = f"{self.entity.name.capitalize()} {atkverb} {target.name}"
